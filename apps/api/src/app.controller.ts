...
export class AppController {
  ...
  @Get('projects/:id/export') @Roles('customer','consultant','admin') async export(@CurrentActor() actor:Actor,@Param('id') id:string,@Res() res:Response){const p=this.store.get(id,actor);if(p.status==='pending'&&actor.role==='customer')return res.status(403).json({message:'顾问介入后才可下载'});const data=await this.excel.generatePdf(p);res.setHeader('Content-Type','application/pdf');res.setHeader('Content-Disposition',`attachment; filename="opening-plan-${p.id}.pdf"`);res.send(data);}
